...
async function getConfig() {
  ...
  const autoFetchUrls = stored.autoFetchUrls === undefined
    ? DEFAULTS.autoFetchUrls
    : Boolean(stored.autoFetchUrls);

  const rawTopN = Number(stored.fetchTopN);
  const fetchTopN = Number.isFinite(rawTopN)
    ? Math.max(0, Math.min(5, Math.floor(rawTopN)))
    : DEFAULTS.fetchTopN;

  const fetchModel = String(stored.fetchModel || "").trim().toLowerCase() || DEFAULTS.fetchModel;

  return {
    providers: providers.length ? providers : DEFAULTS.providers,
    defaultProvider,
    autoFetchUrls,
    fetchTopN,
    fetchModel
...
export async function adminSave(req, res) {
  ...
  const autoFetchUrls = body.autoFetchUrls === undefined
    ? DEFAULTS.autoFetchUrls
    : Boolean(body.autoFetchUrls);

  const rawTopN = Number(body.fetchTopN);
  const fetchTopN = Number.isFinite(rawTopN)
    ? Math.max(0, Math.min(5, Math.floor(rawTopN)))
    : DEFAULTS.fetchTopN;

  const fetchModel = String(body.fetchModel || "").trim().toLowerCase() || DEFAULTS.fetchModel;

  const config = { providers, defaultProvider, autoFetchUrls, fetchTopN, fetchModel };
